...
class RGBLED:
	def __init__(self, pins):
		if len(pins) is not 3:
			sys.exit("RGBLED: Constructor must be called with length-3 array of pin numbers")
	...
	def color_test_thread(self):
		threads = []
		threads.append(threading.Thread(target=self.color_test, args=(self.pins[0], 300, 0.02, 5)))
		threads.append(threading.Thread(target=self.color_test, args=(self.pins[1], 300, 0.035, 5)))
		threads.append(threading.Thread(target=self.color_test, args=(self.pins[2], 300, 0.045, 5)))
		for t in threads:
			t.daemon = True
			t.start()
		for t in threads:
